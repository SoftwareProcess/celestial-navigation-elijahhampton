...
def correct(values = None):
    ...
    #calculate
    correctedDistanceX = int(values['altitude'].split('d')[0]) + (float(values['altitude'].split('d')[1]) / 60)
    ...
    preCorrectedAzimuth = str(radiansToDegrees(acos( (c1 - (c2 * c3)) / (d1 * d2)))) #Should be 82.9490446
    ...
    correctedAzimuthY = round(correctedAzimuthY, 1)
